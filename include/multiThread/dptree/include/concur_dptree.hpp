...
extern void clflush_then_no_fence(volatile void *p);
extern void mfence();
...
namespace dptree
{
    ...
    public:
#ifndef UNIFIED_NODE
        static constexpr int key_capacity = 256; // note: 14
#else
        static constexpr int key_capacity = 14; // node size
#endif
        ...
        struct alignas(64) leaf_node
        {
            struct bitmap
            {
                ...
                inline int alloc_first_unset_bit(size_t h)
                {
                    ...
                    for (int i = 0; i < ints; i++)
                    {
                        int slot = __builtin_ffs(~bits[i]) - 1;
                        if (slot == -1)
                        { // not find in this 64 bits ,goto the next 64 bits
                            continue;
                        }
                        else
                        {
                    ...
                    return -1; // do not find empty slot.
#else
            ...
            preserve_l:
                ...
                while (upsert_kvs_sit != upsert_kvs_eit)
                {
                    ...
                    else
                    {
                        int free_idx =
                            l_alloc_bitmap->alloc_first_unset_bit(hasher(upsert_key));
                        // printf("2.71***************%d,free_idx = %d\n", l_new_meta.count, free_idx);
                        l->pairs[free_idx] = *upsert_kvs_sit;
                        // printf("2.72***************%d\n", l_new_meta.count);
                        l->mark_dirty_cacheline((char *)&l->pairs[free_idx]);
                        // printf("2.73***************%d\n", l_new_meta.count);
                ...
                *last_leaf->next_sibling_ref(nv) = l;
                ...
                //  printf("11***************%d\n", l_new_meta.count);
                //  printf("******************error1!\n\n");
                //  if (l->key_count(nv) != l_alloc_bitmap->popcount())
                //  {
                //      printf("error2!\n");
        ...
        iterator lower_bound(const key_type &key, thread_epoch &e)
        {
            ...
            *reinterpret_cast<uint64_t *>(lookup_key) = __builtin_bswap64(key);
            ...
            while (lo < key_count && cur->ith_key(lo, v) < key)
                ++lo;
            if (lo >= key_count)
                return end();
    ...
    public:
        ...
        void upsert(const key_type &key, const value_type &value)
        {
            ...
            buffer_btree_type *buffer_tree;
            {
                ...
                auto should_insert_f_cpbt = [&key, &value, this](const key_type &key2) -> bool
                {
                    // return true;
                    return false; // update
                };
        ...
        int lookup_range(const key_type &start_key, int scan_size, std::vector<value_type> &values)
        {
            // register reader
        ...
        void do_merge(buffer_btree_type *front_buffer_tree_snapshot)
        {
            ...
            if (is_no_merge(local_ms))
            {
                ...
                if (merge_state.compare_exchange_strong(local_ms, become_pre_merge(local_ms)))
                {
                    ...
                    int thread_id_this = thread_id;                                         // add by zhenxin
                    std::thread([this, local_ms, old_front_buffer_tree, thread_id_this]() { // mfence
#ifdef PIN_CPU
                        pin_cpu_core(thread_id_this);
#endif
                        worker_id = thread_id_this;
                        thread_id = thread_id_this;
