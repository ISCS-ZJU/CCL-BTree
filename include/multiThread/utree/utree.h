...
public:
  ...
  void printAll();
  void insert(entry_key_t, char *, bool is_update);                          // Insert
...
int btree::scan(entry_key_t key, uint64_t len, std::vector<value_type_sob> &buf)
{
  bool f = false;
  char *prev;
  int retry_num = 0;
retry:
  ...
  while (buf.size() < len)
  {
    ...
    else
    {
      ...
      {
        return buf.size();
      } // todo: bug,so we just return.
      // buf.clear();
      // goto retry; //printf("not found.\n");
    }
  }
  return buf.size();
}
...
void btree::insert(entry_key_t key, char *right, bool is_update)
{
  ...
  if (update && prev != NULL)
  {
    if (is_update)
    {
      ...
      __sync_fetch_and_add(&freed_nvm_space, sizeof(list_node_t));
#endif
    }
  }
  else
