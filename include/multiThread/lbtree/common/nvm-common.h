...
{
  clwb(start);
  if (lbtree_getline(start) != lbtree_getline(end)) {
    clwb(end);
  }
}
...
{ 
  unsigned long long start_line= lbtree_getline(start);
  unsigned long long end_line= lbtree_getline(end);
  do {
  ...
  public:
    ...
    */
    void prepareForReverseRead(nl_log_pointer_t *pos)
    {
       ...
       pos->nextline_ptr_= (char *)lbtree_getline(pos->next_ptr_);
...
extern __thread int worker_id;  /* in Thread Local Storage */
