...
static inline void software_clear_cache(void)
{
     ...
     if (buf == NULL)
     {
          perror("malloc in lbtree_clear_cache()");
          exit(1);
     }
     ...
     for (i = 0; i < 10 * MB; i += CACHE_LINE_SIZE) /* write */
          if (buf[i] != 'a')
          {
               printf("Oops? lbtree_clear_cache()\n");
               exit(1);
          }
     free(buf);
}

#define lbtree_clear_cache software_clear_cache

#endif /* lbtree_clear_cache */
