...
#else // dptree前台与后台线程用同一个pool，因此需要cas
...
#pragma GCC optimize("O0")
   void *alloc(unsigned long long size)
   {
      if (mempool_cur + size <= mempool_end)
      {
         register char *p;
         p = mempool_cur;
         while (!__sync_bool_compare_and_swap(&mempool_cur, p, p + size))
         {
            p = mempool_cur;
         };
         memset(p, 0, size);
...
public:
    ...
    */
   // void print_usage(void);
   unsigned long print_usage(void);
...
__thread inline int worker_id = -1; /* in Thread Local Storage */

inline threadMemPools the_thread_mempools;
